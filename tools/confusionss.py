...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.6):
    ...
    def process_batch(self, detections, labels, img_index, file_name):
        ...
        Arguments:
            ...
            labels (Array[M, 5]), class, x1, y1, x2, 
        Returns:
        ...
        if len(detections) > 0:
            ...
            x = torch.where(iou > self.iou_thres)  #self.iou_thres = 0.45
            ...
            if x[0].shape[0]:
                ...
                if x[0].shape[0] > 1:
                    ...
                    matches = matches[np.unique(matches[:, 1], return_index=True)[1]]#np.unique 중복제거
                    matches = matches[matches[:, 2].argsort()[::-1]]
            ...
            for i, gc in enumerate(gt_classes):
                ...
                if n and sum(j) == 1:
                    ...
                    # print(self.matrix[class_index[gc], class_index[int(detection_classes[m1[j]])]])
                    self.matrix[class_index[gc], class_index[int(detection_classes[m1[j]])]] += 1  # correct
                    # print(f"---------------------------------GC: {gc}")
                    
                    if gc.item() != detection_classes[m1[j]].item(): 
                            log_list = [img_index, file_name, gc.item(), int(detection_classes[m1[j]].item()), detections[int(matches.item((idx, 1))), 4].item(), 'False', '', self.matrix[class_index[int(detection_classes[m1[j]])], class_index[gc]].item()]
                            # print(log_list)
                        ...
                        log_list = [img_index, file_name, gc.item(), int(detection_classes[m1[j]].item()), detections[int(matches.item((idx, 1))), 4].item(), 'TRUE', self.matrix[class_index[int(detection_classes[m1[j]])], class_index[gc]].item(), '']
                        # print(log_list)
                        self.csv.append(log_list)
                    ...
                    self.matrix[self.nc, class_index[gc]] += 1  # background FP
                    log_list = [img_index, file_name, gc.item(), self.nc, '', 'FP', '', self.matrix[self.nc, class_index[gc]].item()]
                    self.csv.append(log_list)
            ...
            if n:
                for i, dc in enumerate(detection_classes):
                    if not any(m1 == i):
                        ...
                        # print(int(dc))
                        # print(952)
                        self.matrix[class_index[int(dc)], self.nc] += 1  # background FN
                        # print(f"---------------------------------DC: {int(dc)}")
                        log_list = [img_index, file_name, self.nc, dc.item(), '', 'FN', '', self.matrix[class_index[int(dc)], self.nc].item()]
                        self.csv.append(log_list)
    ...
    def plot(self, save_dir='', names=(), rec_or_pred=0):
        try:
            ...
            with warnings.catch_warnings():
                ...
                sbp = sn.heatmap(self.matrix, annot=self.nc < 40, annot_kws={"size": 5}, cmap='Blues', fmt='d', square=True,
                                xticklabels=names + ['NaN'] if labels else "auto",
                                yticklabels=names + ['NaN'] if labels else "auto").set_facecolor((1, 1, 1))
            
            fig.axes[0].set_xlabel('Ground Trurh')
...
def box_iou(box1, box2):
    ...
    if isinstance(box1, np.ndarray):
        box3 = torch.from_numpy(box1)
    else:
        box3 = box1
    if isinstance(box2, np.ndarray):
        box4 = torch.from_numpy(box2)
    else:
        box4 = box2
    ...
    area1 = box_area(box3.T)
    area2 = box_area(box4.T)

    # inter(N,M) = (rb(N,M,2) - lt(N,M,2)).clamp(0).prod(2)
    inter = (torch.min(box3[:, None, 2:], box4[:, 2:]) - torch.max(box3[:, None, :2], box4[:, :2])).clamp(0).prod(2)
...
def xywh2xyxy(x):
    ...
    y[:, 0] = x[:, 0]  #가공툴의 x는 맨 왼쪽 위에 있는 x
    y[:, 1] = x[:, 1]  #가공툴의 y는 맨 왼쪽 위에 있는 y
...
def process_batch(detections, labels, iouv):
    ...
    iou = box_iou(labels[:, 1:], detections[:, :4])
    x = torch.where((iou >= iouv[0]) & (labels[:, 0:1] == detections[:, 5]))  # IoU above threshold and classes match #ch
    if x[0].shape[0]:
