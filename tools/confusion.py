...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.6): ## iconfess
        self.matrix = np.zeros(shape=(nc + 1, nc + 1), dtype=np.int32)
        self.nc = nc  # number of classes
        self.conf = conf
        self.iou_thres = iou_thres
        self.csv = [['img_id', 'file_name', 'GT class', 'prediction class', 'confidence', 'Is correct', 'accumulated TRUE', 'accumulated FALSE']]

    def process_batch(self, detections, labels, img_index, file_name):#
        """
            ...
            for i, gc in enumerate(gt_classes):
                ...
                if n and sum(j) == 1:
                    ...
                    if gc.item() != detection_classes[m1[j]].item():
                        log_list = [img_index, file_name, gc.item(), detection_classes[m1[j]].item(), detections[int(matches.item((idx, 1))), 4].item(), 'FALSE', '', self.matrix[detection_classes[m1[j]], gc].item()]
                        self.csv.append(log_list)
                    else:
                        log_list = [img_index, file_name,gc.item(), detection_classes[m1[j]].item(), detections[int(matches.item((idx, 1))), 4].item(), 'TRUE', self.matrix[detection_classes[m1[j]], gc].item(), '']
                        self.csv.append(log_list)
                        # print("---------------->>> gt: {} - dt: {}".format(gc.item(), detection_classes[m1[j]].item()))
                        # log_list = [img_index, gc.item(), detections[int(matches.item((idx, 1))), 5].item(), detections[int(matches.item((idx, 1))), 4].item(), 'TP', self.matrix[detection_classes[m1[j]], gc].item(), '', '']
                        # self.csv.append(log_list)
                        # sleep(0.005)
                        # print("---------------->>> gt: {} - dt: {}".format(gc.item(), detection_classes[m1[j]].item()))
                        ...
                        # print("---->>> Log List: & IoU:", log_list, matches[idx, 2])
                        # print("--->>> img_id: {} --GroundTruth: {} --Predict: {} --IoU: {:.3f} --Confidence: {:.3f} --Confusion Matrix: {} --accumulated TP: {}"
                        # .format(log_list[0], log_list[1], log_list[2], matches[idx, 2], log_list[3], log_list[4], log_list[5]))
                        
                        # if gc.item() != detection_classes[m1[j]].item():
                        #     print("====!!!!!!!!!!gt: {} - dt: {}".format(gc.item(), detection_classes[m1[j]].item()))
                        # print("-------confidence: {}", detections[int(matches.item((idx, 1))), 4].item())
                        # print("------------m0: {} m1:{}".format(m0[i].item(), m1[i].item()))
                        # print("============gt: {} - dt: {}".format(matches.item((idx,0)), matches.item((idx, 1))))
                        idx += 1
                    # if list(detections.size())[0] > detection_classes[m1[j]].item():
                    #     if gc.item() != detection_classes[m1[j]].item():
                    #         log_list = [img_index, gc.item(), detection_classes[m1[j]].item(), detections[int(matches.item((idx, 1))), 4].item(), 'FALSE', '', self.matrix[detection_classes[m1[j]], gc].item()]
                    #         self.csv.append(log_list)
                    #     else:
                    #         log_list = [img_index, gc.item(), detection_classes[m1[j]].item(), detections[int(matches.item((idx, 1))), 4].item(), 'TRUE', self.matrix[detection_classes[m1[j]], gc].item(), '']
                    #         self.csv.append(log_list)
                    #     # print("---------------->>> gt: {} - dt: {}".format(gc.item(), detection_classes[m1[j]].item()))
                    #     # log_list = [img_index, gc.item(), detections[int(matches.item((idx, 1))), 5].item(), detections[int(matches.item((idx, 1))), 4].item(), 'TP', self.matrix[detection_classes[m1[j]], gc].item(), '', '']
                    #     # self.csv.append(log_list)
                    ...
                    # # print("-------------aaaa: ", xx)
                    # # if img_index < 5:
                    # #     # arr = detection_classes.numpy()
                    # #     # # print("---j:", j)
                    # #     # print("+++++++ m1: ", arr[i])
                    # #     print("<<<<<<<--id: {} is DT_{}: GT_{} --- TP".format(img_index, detection_classes[m1[j]].item(), gc))
                    # #     # print("<<<<<<<<<<<<<<<<  i - confidence - xx: ", i, detections[i, 4], xx)
                    # #     # print("<<<<<<<<<<<<<<<< confidence : ", detections[detection_classes[m1[j]].item(), 4].item())
                    # #     # print("-----id {} TP: {} ".format(img_index, self.matrix) )
                else:
                    self.matrix[self.nc, gc] += 1  # background FP
                    log_list = [img_index, file_name, gc.item(), self.nc, '', 'FP', '', self.matrix[self.nc, gc].item()]
                    self.csv.append(log_list)      
            if n:
                for i, dc in enumerate(detection_classes):
                    if not any(m1 == i):
                        self.matrix[dc, self.nc] += 1  # background FN
                        log_list = [img_index, file_name, self.nc, dc.item(), '', 'FN', '', self.matrix[dc, self.nc].item()]
                        self.csv.append(log_list)
    ...
    def plot(self, normalize=True, save_dir='', names=(), rec_or_pred=0):#0=recall, 1=pred
        try:
            ...
            sn.set(font_scale=0.8 if self.nc < 20 else 0.6)  # for label size
            # labels = (0 < len(names) < 602) and len(names)+351 == self.nc  # apply names to ticklabels
            labels = True
            ...
            with warnings.catch_warnings():
                ...
                sbp = sn.heatmap(self.matrix, annot=True, annot_kws={"size": 0.6}, cmap='Blues', fmt='s', square=True,  #self.nc < 40
                             xticklabels=names + ['NaN'] if labels else "auto",
                             yticklabels=names + ['NaN'] if labels else "auto").set_facecolor((1, 1, 1))
